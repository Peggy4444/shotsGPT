...
class Passes(Data):
    ...
    # def get_feature_contributions_xNN(self, competition,scale_output=False):
    ...
    #     # Ensure X_h is a DataFrame with correct column names
    #     explainer = shap.Explainer(model_predict, X_h)
    #     shap_vals = explainer(X_h)

    #         # SHAP array and feature names
    #     shap_array = shap_vals.values  # shape: (n_samples, n_h_features)
    #     h_names = X_h.columns.tolist()

    #     # print("🔍 SHAP array sample for each h-feature:")
    #     # for i, h in enumerate(h_names):
    #     #     col_vals = shap_array[:, i]
    #     #     print(f"{h}: min={col_vals.min():.6f}, max={col_vals.max():.6f}, mean={col_vals.mean():.6f}")

    #     # Build logistic_coeffs from relevant model parameter CSVs
    #     logistic_dfs = [
    ...
    #     if scale_output:
    #     #print("⚠️ Scaling contributions by 1000 for readability")
    #         for col in base_contrib_matrix.columns:
    #             base_contrib_matrix[col] *= 1000
